...
CONFIG = {
    ...
    "OUTPUT_EXCEL_PATH": "Study3_qwen3.xlsx",
    "SHEET_NAME": "Sheet1",
    "CHAT_CONTENT_COLUMN": "merged_messages",
    "PROLIFIC_ID_COLUMN": "PROLIFIC_PID",  # Column name configuration
    "MAX_RETRIES": 3,
    "API_TIMEOUT": 60,
    "QWEN_MODEL": "qwen-plus",
    "PARALLEL_WORKERS": 10,  # Number of parallel worker threads
    "OUTPUT_BATCH_SIZE": 100  
}


def initialize_qwen_client():
    """Initialize Qwen API client with custom base URL"""
    return OpenAI(
...
def rate_chat_with_qwen(client: OpenAI, chat_content: str, prolific_id: str):
    """Call Qwen model to get ratings, return None on failure"""
...
def clean_response_text(text):
    ...
    # Remove JSON wrapping markers (e.g., {"name":"JSONPlugin"...)
...
def parse_rating_response(response_text: str):
    ...
    try:
        ...
        for dimension in ["competence", "warmth", "trustworthiness"]:
            ...
            if not (isinstance(value, int) or isinstance(value, float)) or not (1 <= value <= 7):
        ...
        print(f"JSON decoding error: {str(e)}, cleaned response: {cleaned_text[:50]}...")
        return None, f"JSON decoding error: {str(e)}"
    except Exception as e:
        print(f"Parsing failed: {str(e)}, cleaned response: {cleaned_text[:50]}...")
...
def process_row(client, row, save_dir):
    ...
    print(f"Processing (PROLIFIC_PID: {prolific_id}): {content[:50]}...")
    response = rate_chat_with_qwen(client, content, prolific_id)

    # Save raw API response
    api_response_path = os.path.join(save_dir, f"api_response_{prolific_id}.txt")
    ...
    if ratings:
        # Validate PROLIFIC_PID match
        if ratings["prolific_pid"] != prolific_id:
            print(f"Warning: Mismatched PROLIFIC_PID (expected: {prolific_id}, received: {ratings['prolific_pid']})")

        print(f"Rating success (PROLIFIC_PID: {prolific_id}): {ratings}")
        return row.name, ratings
    else:
        print(f"Rating failed (PROLIFIC_PID: {prolific_id}): {error if error else 'Unknown error'}")
        return row.name, {
            "competence": None,
            "warmth": None,
            "trustworthiness": None,
            "reason": error if error else "Parsing failed",
            "prolific_pid": prolific_id
        }
...
def process_chat_ratings():
    ...
    # Create directory to save API responses
    save_dir = "/Users/huangxinjie/Desktop/personality/save"
    ...
    required_columns = [CONFIG["PROLIFIC_ID_COLUMN"], CONFIG["CHAT_CONTENT_COLUMN"]]
    ...
    for batch_start in range(0, total_rows, CONFIG["OUTPUT_BATCH_SIZE"]):
        ...
        print(f"Processed {processed_count}/{total_rows} records, saving intermediate results...")
        excel_columns = [
            CONFIG["PROLIFIC_ID_COLUMN"],
            CONFIG["CHAT_CONTENT_COLUMN"],
            "competence", "warmth", "trustworthiness", "reason"
        ]
        df[excel_columns].to_excel(CONFIG["OUTPUT_EXCEL_PATH"], index=False)

    progress_bar.close()

    # Final save
    print(f"\n✅ All records processed. Results saved to: {CONFIG['OUTPUT_EXCEL_PATH']}")
